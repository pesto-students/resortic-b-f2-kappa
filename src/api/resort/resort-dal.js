...
class ResortDAL {
  ...
  searchResorts = async (req, res) => {
    ...
    const data = await database.query(
      "SELECT  (SELECT SUM(quantity) FROM heroku_6c4649b1a5324b1.roomtables AS rm JOIN heroku_6c4649b1a5324b1.resortroomtables AS rr ON rm.id = rr.roomtableId WHERE resorttableId = r.id ) - SUM(IFNULL(rooms_count,0)) AS availableRooms ,r.id AS id, r.resort_name AS resort_name, r.city, r.starting_price,r.major_aminities FROM heroku_6c4649b1a5324b1.bookingtables AS outbook RIGHT JOIN heroku_6c4649b1a5324b1.resorttables AS r ON outbook.resorttableId = r.id WHERE ((IFNULL(check_in, $1) BETWEEN  $1 AND $2) OR  (IFNULL(check_out, $1) BETWEEN  $1 AND $2)) AND r.city LIKE $3 GROUP BY r.id HAVING availableRooms > $4;",
