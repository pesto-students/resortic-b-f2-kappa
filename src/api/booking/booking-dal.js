...
class BookingDAL {
  ...
  getUserPastBookings = async (req, res) => {
    ...
    // );
    const data = await database.query(
      "SELECT p.paid_amount, b.id, b.check_in,b.check_out,b.guests,b.rooms_count,b.usertableId,r.resort_name,r.address, b.status,(SELECT AVG(rating) FROM reviewtables WHERE resorttableId = r.id) as rating FROM paymenttables as p JOIN bookingtables as b ON p.bookingtableId = b.id JOIN resorttables as r ON b.resorttableId = r.id WHERE b.usertableId=$1 AND (b.check_in < curdate() or b.status = 'Cancelled')  AND b.is_deleted=FALSE",
      { type: Sequelize.QueryTypes.SELECT, bind: [sUserId] }
    );
    return data;
  };
