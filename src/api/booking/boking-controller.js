...
const schema = require("../../validation/bookingValidationSchema")
...
class BookingController {
  newBooking = async (req, res) => {
    try {
      ...
      // }
      const value = await schema.createBook.validateAsync(req.body);
      console.log(value);

      return await BookingService._book(req, res);
    } catch (error) {
      res.status(400).json({ success: false, msg3: error });
    }
  };
